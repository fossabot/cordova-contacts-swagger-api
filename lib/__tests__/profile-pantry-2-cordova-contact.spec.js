...
const profilePantry2CordovaContact = require('../index');
...
describe('adapts profile-pantry employee attributes to Cordova Contact attributes, including', () => {
  ...
  beforeEach(() => {
    contact = profilePantry2CordovaContact.adapt(profilePantryEmployee);
  });
  ...
  it('emails {array<ContactField>}: The Contact\'s work e-mail addresses', () => {
    ...
    expect(contact.emails[0].pref).toBe(true);
    expect(contact.emails[0].type).toBe('external-email');
    expect(contact.emails[1].pref).toBe(false);
    expect(contact.emails[1].type).toBe('internal-email');
  });
  ...
  it('organizations {array<ContactOrganization>}: All the contact\'s organizations', () => {
    ...
    expect(contact.organizations[0].department).toBe(profilePantryEmployee.financeLobName);
  ...
  it('phoneNumbers {array<ContactField>}: The contact\'s work phone numbers', () => {
    ...
    expect(contact.phoneNumbers[0].type).toBe('mobile');
    expect(contact.phoneNumbers[0].pref).toBe(false);
    expect(contact.phoneNumbers[1].type).toBe('office-phone-number');
    expect(contact.phoneNumbers[1].pref).toBe(true);
  });
  ...
  it('urls {array<ContactField>}: An array of web pages associated with the contact', () => {
    ...
    expect(contact.urls.length).toBe(4);
  });
});
