...
const { CordovaContactAdaptor, CordovaContactType } = require('../index');

describe('adapts profile-pantry employee attributes to Cordova Contact attributes, including', () => {
  let contact;

  beforeEach(() => {
    contact = CordovaContactAdaptor.adapt(profilePantryEmployee);
  });
  ...
  it('urls {array<ContactField>}: An array of web pages associated with the contact', () => {
    ...
    expect(contact.urls.length).toBe(5);
  });
});
